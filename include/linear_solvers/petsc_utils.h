#ifndef __PETSC_UTILS_H
#define __PETSC_UTILS_H
...
namespace PETScUtils {
  ...
  // P-Adaptive stuff
  // Copy PETSc vec array to OP2 dat
  void copy_vec_to_dat_p_adapt(op_dat dat, const double *dat_d, DGMesh2D *mesh);

  // Copy OP2 dat to PETSc vec array
  void copy_dat_to_vec_p_adapt(op_dat dat, double *dat_d, DGMesh2D *mesh);

  // Load a PETSc vector with values from an OP2 dat for CPUs
  void load_vec_p_adapt(Vec *v, op_dat v_dat, DGMesh2D *mesh);

  // Load an OP2 dat with the values from a PETSc vector for CPUs
  void store_vec_p_adapt(Vec *v, op_dat v_dat, DGMesh2D *mesh);
