...
void poisson_set_rhs_blas(INSData *nsData) {
  ...
  op_mpi_halo_exchanges_cuda(nsData->cells, 1, poisson_rhs_args);

  cublas_poisson_set_rhs(handle, nsData->numCells, (double *)nsData->rhs->data_d);

  // Set correct dirty bits for OP2
  op_mpi_set_dirtybit_cuda(1, poisson_rhs_args);
