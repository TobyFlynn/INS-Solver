...
//user function
__device__ void set_rhs2_gpu( const double *J, const double *x, const double *y, double *rhs) {
  for(int i = 0; i < 15; i++) {
    if(x[i] == 0.0) {
      rhs[i] = 0.5;
    } else if(x[i] == 1.0) {
...
__global__ void op_cuda_set_rhs2(
...
void op_par_loop_set_rhs2(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_set_rhs2<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
