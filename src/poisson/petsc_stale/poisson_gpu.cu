...
void PetscPoissonStaleSolve::copy_vec_to_dat(op_dat dat, const double *dat_d) {
  op_arg copy_args[] = {
...
void PetscPoissonStaleSolve::copy_dat_to_vec(op_dat dat, double *dat_d) {
  op_arg copy_args[] = {
...
void PetscPoissonStaleSolve::create_vec(Vec *v) {
...
void PetscPoissonStaleSolve::destroy_vec(Vec *v) {
  VecDestroy(v);
}

// Load a PETSc vector with values from an OP2 dat for GPUs
void PetscPoissonStaleSolve::load_vec(Vec *v, op_dat v_dat) {
  double *v_ptr;
...
void PetscPoissonStaleSolve::store_vec(Vec *v, op_dat v_dat) {
  const double *v_ptr;
...
void PetscPoissonStaleSolve::setMatrix() {
  // if(pMatInit)
  //   MatDestroy(&pMat);
  if(!pMatInit) {
