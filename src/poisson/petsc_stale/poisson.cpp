...
PetscPoissonStaleSolve::PetscPoissonStaleSolve(DGMesh *m, INSData *nsData, LS *s) {
  mesh = m;
  data = nsData;
  ls = s;
...
PetscPoissonStaleSolve::~PetscPoissonStaleSolve() {
  free(u_data);
  free(rhs_data);
  free(in_data);
  free(out_data);
...
void PetscPoissonStaleSolve::init() {
  mat->init();
...
bool PetscPoissonStaleSolve::solve(op_dat b_dat, op_dat x_dat) {
  if(!vec_created) {
    create_vec(&b);
    create_vec(&x);
    vec_created = true;
...
void PetscPoissonStaleSolve::calc_rhs(const double *u_d, double *rhs_d) {
  // Copy u to OP2 dat
...
void PetscPoissonStaleSolve::precond(const double *in_d, double *out_d) {
...
void PetscPoissonStaleSolve::setDirichletBCs(int *d) {
  mat->setDirichletBCs(d);
}

void PetscPoissonStaleSolve::setNeumannBCs(int *n) {
  mat->setNeumannBCs(n);
}

void PetscPoissonStaleSolve::setBCValues(op_dat bc) {
  bc_dat = bc;
}

double PetscPoissonStaleSolve::getAverageConvergeIter() {
...
PetscPressureStaleSolve::PetscPressureStaleSolve(DGMesh *m, INSData *d, LS *s) : PetscPoissonStaleSolve(m, d, s) { }

void PetscPressureStaleSolve::set_refresh_num(int rn) {
  refresh_every = rn;
  num_iter_since_refresh = rn;
}

void PetscPressureStaleSolve::setup() {
  if(num_iter_since_refresh >= refresh_every) {
    mat->update_glb_ind();
    ...
    num_iter_since_refresh = 0;
  }

  num_iter_since_refresh++;
}
