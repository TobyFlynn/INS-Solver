...
void poisson_op5_omp4_kernel(
  ...
  for ( int e=start; e<end; e++ ){
    ...
    const double *gFactor = &data10[21 * map5idx];
    const double *factor = &data11[15 * map5idx];
    double *op = &data12[105*n_op];
    ...
    if(*edgeType != *d0 && *edgeType != *d1 && *edgeType != *d2) {
      ...
      for(int i = 0; i < 7 * 15; i++) {
        ...
        op[i] = gVM[indT] * gaussW_g_ompkernel[i % 7] * sJ[indSJ] * tauA[i % 7]
                - factor[indFactor] * mD[indT] * gaussW_g_ompkernel[i % 7] * sJ[indSJ];
      }
    }
    //end inline func
  }
