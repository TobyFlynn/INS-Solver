...
void pressure_solve_1_omp4_kernel(
  ...
  for ( int e=start; e<end; e++ ){
    ...
    const double *gRhoL = &data26[21 * map2idx];
    const double *gRhoR = &data26[21 * map3idx];
    const double *rhoL = &data28[15 * map2idx];
    const double *rhoR = &data28[15 * map3idx];
    const double *uL = &data30[15 * map2idx];
    const double *uR = &data30[15 * map3idx];
    ...
    for(int i = 0; i < 15; i++) {
      for(int j = 0; j < 15; j++) {
        ...
        for(int k = 0; k < 7; k++) {
          ...
          op1L[c_ind] += -(1.0 / rhoL[i]) * mDL[a_ind] * gaussW_g_ompkernel[k]
                         * sJL[factors_indL] * gVML[b_ind];
          op1R[c_ind] += -(1.0 / rhoR[i]) * mDR[a_ind] * gaussW_g_ompkernel[k]
                         * sJR[factors_indR] * gVMR[b_ind];

          op2L[c_ind] += (1.0 / rhoL[i]) * mDL[a_ind] * gaussW_g_ompkernel[k]
                         * sJL[factors_indL] * gVPL[b_ind];
          op2R[c_ind] += (1.0 / rhoR[i]) * mDR[a_ind] * gaussW_g_ompkernel[k]
