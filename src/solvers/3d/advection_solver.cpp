...
AdvectionSolver3D::AdvectionSolver3D(DGMesh3D *m) {
  ...
  f = op_decl_dat(mesh->cells, DG_NP, "double", data_t0, "advection_f");
  g = op_decl_dat(mesh->cells, DG_NP, "double", data_t0, "advection_g");
  h = op_decl_dat(mesh->cells, DG_NP, "double", data_t0, "advection_h");
  rk[0] = op_decl_dat(mesh->cells, DG_NP, "double", data_t0, "advection_rk0");
  rk[1] = op_decl_dat(mesh->cells, DG_NP, "double", data_t0, "advection_rk1");
  rk[2] = op_decl_dat(mesh->cells, DG_NP, "double", data_t0, "advection_rk2");
  rkQ   = op_decl_dat(mesh->cells, DG_NP, "double", data_t0, "advection_rkQ");
  free(data_t0);

  double *data_t1 = (double *)calloc(DG_NUM_FACES * DG_NPF * mesh->cells->size, sizeof(double));
  flux = op_decl_dat(mesh->cells, DG_NUM_FACES * DG_NPF, "double", data_t1, "advection_flux");
  free(data_t1);
...
void AdvectionSolver3D::rhs(op_dat val, op_dat u, op_dat v, op_dat w, op_dat val_out) {
  ...
  op_par_loop(advec_3d_flux, "advec_3d_flux", mesh->faces,
              ...
              op_arg_dat(val,  -2, mesh->face2cells, DG_NP, "double", OP_READ),
              op_arg_dat(u,    -2, mesh->face2cells, DG_NP, "double", OP_READ),
              op_arg_dat(v,    -2, mesh->face2cells, DG_NP, "double", OP_READ),
  ...
  if(mesh->bface2cells) {
    op_par_loop(advec_3d_bflux, "advec_3d_bflux", mesh->bfaces,
                ...
                op_arg_dat(val,  0, mesh->bface2cells, DG_NP, "double", OP_READ),
                op_arg_dat(u,    0, mesh->bface2cells, DG_NP, "double", OP_READ),
                op_arg_dat(v,    0, mesh->bface2cells, DG_NP, "double", OP_READ),
                op_arg_dat(w,    0, mesh->bface2cells, DG_NP, "double", OP_READ),
                op_arg_dat(flux, 0, mesh->bface2cells, DG_NUM_FACES * DG_NPF, "double", OP_INC));
  }
...
void AdvectionSolver3D::set_dt(const double t) {
  dt = t;
  op_printf("Advection dt is %g\n", dt);
}
