...
KDTreeMPI::KDTreeMPI(DGMesh2D *m, const DG_FP alpha) : KDTree(m) {
  timer->startTimer("KDTreeMPI - init");

  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &comm_size);

  const int block_lengths[] = {2,1};
  ...
  DG_FP *bb_buf = (DG_FP *)calloc(comm_size * 4, sizeof(DG_FP));
  DG_FP snd_buf[] = {min_x, max_x, min_y, max_y};
  MPI_Allgather(snd_buf, 4, DG_MPI_FP, bb_buf, 4, DG_MPI_FP, MPI_COMM_WORLD);

  ranks.clear();
  const DG_FP min_l[] = {min_x, min_y};
  const DG_FP max_l[] = {max_x, max_y};
  for(int i = 0; i < comm_size; i++) {
    if(i == rank) continue;

    const DG_FP i_min_x = bb_buf[i * 4];
    const DG_FP i_max_x = bb_buf[i * 4 + 1];
    const DG_FP i_min_y = bb_buf[i * 4 + 2];
    const DG_FP i_max_y = bb_buf[i * 4 + 3];

    const DG_FP min_i[] = {i_min_x, i_min_y};
    const DG_FP max_i[] = {i_max_x, i_max_y};
  ...
  timer->endTimer("KDTreeMPI - init");
}

DG_FP KDTreeMPI::min_dist_bb(const DG_FP *min_0, const DG_FP *max_0,
...
void KDTreeMPI::build_tree(const DG_FP *x, const DG_FP *y, const int num, op_dat s) {
  timer->startTimer("KDTreeMPI - pre_build_setup");
  pre_build_setup(x, y, num, s);
  timer->endTimer("KDTreeMPI - pre_build_setup");

  timer->startTimer("KDTreeMPI - comm - 0");
  ...
  timer->endTimer("KDTreeMPI - comm - 0");
  timer->startTimer("KDTreeMPI - packing");
  ...
  const int POLY_NUM_COEFF = PolyApprox::num_coeff();
  vector<DG_FP> poly_coeff_snd(polys.size() * (POLY_NUM_COEFF + 2));
  for(int i = 0; i < polys.size(); i++) {
    int ind = i * (POLY_NUM_COEFF + 2);
    for(int j = 0; j < POLY_NUM_COEFF; j++) {
      poly_coeff_snd[ind + j] = polys[i].get_coeff(j);
    }
    DG_FP off_x, off_y;
    polys[i].get_offsets(off_x, off_y);
    poly_coeff_snd[ind + POLY_NUM_COEFF] = off_x;
    poly_coeff_snd[ind + POLY_NUM_COEFF + 1] = off_y;
  }
  timer->endTimer("KDTreeMPI - packing");

  timer->startTimer("KDTreeMPI - comm - 1");
  // Send/Recieve
  vector<kdtree_mpi_pack> pts_rcv(num_pts_g);
  vector<DG_FP> polys_coeff_rcv(num_polys_g * (POLY_NUM_COEFF + 2));
  ...
  for(int i = 0; i < ranks.size(); i++) {
    ...
    MPI_Irecv(&polys_coeff_rcv[num_polys_per_rank_disp[i] * (POLY_NUM_COEFF + 2)], num_polys_per_rank[i] * (POLY_NUM_COEFF + 2), DG_MPI_FP, ranks[i], 0, MPI_COMM_WORLD, &polys_coeff_mpi_req_rcv[i]);
  }

  for(int i = 0; i < ranks.size(); i++) {
    MPI_Isend(packed_data.data(), n, packed_type, ranks[i], 0, MPI_COMM_WORLD, &pts_mpi_req_snd[i]);
    MPI_Isend(poly_coeff_snd.data(), polys.size() * (POLY_NUM_COEFF + 2), DG_MPI_FP, ranks[i], 0, MPI_COMM_WORLD, &polys_coeff_mpi_req_snd[i]);
  }
  ...
  timer->endTimer("KDTreeMPI - comm - 1");

  timer->startTimer("KDTreeMPI - unpack");
  // Unpack data
  int curr_rank = 0;
  ...
  for(int i = 0; i < num_polys_g; i++) {
    ...
    PolyApprox p(c, off_x, off_y);
    polys.push_back(p);
  }
  timer->endTimer("KDTreeMPI - unpack");
