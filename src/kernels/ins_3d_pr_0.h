inline void ins_3d_pr_0(const DG_FP *t, const int *bc_type, const int *faceNum,
  ...
  if(*bc_type == BC_TYPE_NO_SLIP || *bc_type == BC_TYPE_SLIP) {
    ...
    if(tmp_bc_type == BC_NEUMANN) {
      for(int i = 0; i < DG_NPF; i++) {
        const int fmask_ind = fmask[i];
        DG_FP bc_neumann = ps3d_custom_bc_get_pr_neumann(*bc_type, *t, x[fmask_ind], y[fmask_ind], z[fmask_ind],
                              *nx, *ny, *nz, n0[fmask_ind], n1[fmask_ind], n2[fmask_ind], curl20[fmask_ind],
                              curl21[fmask_ind], curl22[fmask_ind], r_ynolds);
