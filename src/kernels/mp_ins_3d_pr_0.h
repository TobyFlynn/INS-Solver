inline void mp_ins_3d_pr_0(const double *t, const int *bc_type, const int *faceNum,
                           ...
                           const double *curl21, const double *curl22,
  ...
  if(*bc_type == 0 || *bc_type == 2) {
    for(int i = 0; i < DG_NPF; i++) {
      double res0 = -n0[fmaskB[i]] - curl20[fmaskB[i]] / (r_ynolds * rho[fmaskB[i]]);
      double res1 = -n1[fmaskB[i]] - curl21[fmaskB[i]] / (r_ynolds * rho[fmaskB[i]]);
      double res2 = -n2[fmaskB[i]] - curl22[fmaskB[i]] / (r_ynolds * rho[fmaskB[i]]);
