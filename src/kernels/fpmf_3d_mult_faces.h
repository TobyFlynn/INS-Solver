inline void fpmf_3d_mult_faces(const int **order, const int *faceNum, const int *fmaskL_corrected,
  ...
  for(int j = 0; j < dg_npf; j++) {
    ...
    const DG_FP diffL_u_x = nx[0] * (factor[1][fmaskR_corrected[j]] * in_x[1][fmaskR_corrected[j]] + factor[0][fmaskL[j]] * in_x[0][fmaskL[j]]);
    const DG_FP diffL_u_y = ny[0] * (factor[1][fmaskR_corrected[j]] * in_y[1][fmaskR_corrected[j]] + factor[0][fmaskL[j]] * in_y[0][fmaskL[j]]);
    const DG_FP diffL_u_z = nz[0] * (factor[1][fmaskR_corrected[j]] * in_z[1][fmaskR_corrected[j]] + factor[0][fmaskL[j]] * in_z[0][fmaskL[j]]);
    ...
    const DG_FP l_tmpL = factor[0][fmaskL[j]] * 0.5 * sJ[0] * -diffL_u;
    ...
    const DG_FP diffR_u_x = nx[1] * (factor[1][fmaskR[j]] * in_x[1][fmaskR[j]] + factor[0][fmaskL_corrected[j]] * in_x[0][fmaskL_corrected[j]]);
    const DG_FP diffR_u_y = ny[1] * (factor[1][fmaskR[j]] * in_y[1][fmaskR[j]] + factor[0][fmaskL_corrected[j]] * in_y[0][fmaskL_corrected[j]]);
    const DG_FP diffR_u_z = nz[1] * (factor[1][fmaskR[j]] * in_z[1][fmaskR[j]] + factor[0][fmaskL_corrected[j]] * in_z[0][fmaskL_corrected[j]]);
    ...
    const DG_FP l_tmpR = factor[1][fmaskR[j]] * 0.5 * sJ[1] * -diffR_u;
