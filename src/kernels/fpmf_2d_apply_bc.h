inline void fpmf_2d_apply_bc(const int *p, const int *faceNum, const int *bc_type,
  ...
  if(*bc_type == 0) {
    ...
    for(int i = 0; i < dg_np; i++) {
      for(int j = 0; j < dg_np; j++) {
        ...
        D[ind] = *nx * (geof[RX_IND] * dr_mat[ind] + geof[SX_IND] * ds_mat[ind]);
        D[ind] += *ny * (geof[RY_IND] * dr_mat[ind] + geof[SY_IND] * ds_mat[ind]);
        D[ind] *= fact[i];
      }
    }
