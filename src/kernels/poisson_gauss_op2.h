inline void poisson_gauss_op2(const int **p, const DG_FP *gF0Dr,
                              ...
                              DG_FP *op1L, DG_FP *op1R, DG_FP *op2L,
                              DG_FP *op2R) {
  int edgeL = edgeNum[0];
  int edgeR = edgeNum[1];
  ...
  for(int i = 0; i < DG_GF_NP; i++) {
    ...
    tauL[i] = 0.5 * fmax((p[0][0] + 1) * (p[0][0] + 2) * max_hinv, (p[1][0] + 1) * (p[1][0] + 2) * max_hinv);
  ...
  for(int i = 0; i < DG_GF_NP; i++) {
    ...
    tauR[i] = 0.5 * fmax((p[0][0] + 1) * (p[0][0] + 2) * max_hinv, (p[1][0] + 1) * (p[1][0] + 2) * max_hinv);
