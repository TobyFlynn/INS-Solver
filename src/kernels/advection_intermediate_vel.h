inline void advection_intermediate_vel(const double *a0, const double *a1,
                                       ...
                                       const double *N1Old, const double *st0,
                                       const double *st1, const double *st0Old,
                                       const double *st1Old, double *q0T,
                                       double *q1T) {
  ...
  for(int i = 0; i < DG_NP; i++) {
    ...
    q0T[i] = (*a0 * q0[i] + *a1 * q0Old[i]) - *dt * (*b0 * (N0[i] + st0[i]) + *b1 * (N0Old[i] + st0Old[i]));
    q1T[i] = (*a0 * q1[i] + *a1 * q1Old[i]) - *dt * (*b0 * (N1[i] + st1[i] + gravity) + *b1 * (N1Old[i] + st1Old[i] + gravity));
  }
}
