inline void advection_faces(const int *edgeNum, const bool *rev,
                            ...
                            const double **sJ, double **exQ0, double **exQ1) {
  ...
  for(int i = 0; i < DG_GF_NP; i++) {
    ...
    // Numerical flux LHS
    exQ0[0][lInd] += 0.5 * gaussW_g[i] * sJ[0][lInd] * (-nx[0][lInd] * (lF0 - rF0) - ny[0][lInd] * (lF1 - rF1) - maxVel * (q0[1][rInd] - q0[0][lInd]));
    exQ1[0][lInd] += 0.5 * gaussW_g[i] * sJ[0][lInd] * (-nx[0][lInd] * (lF2 - rF2) - ny[0][lInd] * (lF3 - rF3) - maxVel * (q1[1][rInd] - q1[0][lInd]));
    // Numerical flux RHS
    exQ0[1][rInd] += 0.5 * gaussW_g[i] * sJ[1][rInd] * (-nx[1][rInd] * (rF0 - lF0) - ny[1][rInd] * (rF1 - lF1) - maxVel * (q0[0][lInd] - q0[1][rInd]));
    exQ1[1][rInd] += 0.5 * gaussW_g[i] * sJ[1][rInd] * (-nx[1][rInd] * (rF2 - lF2) - ny[1][rInd] * (rF3 - lF3) - maxVel * (q1[0][lInd] - q1[1][rInd]));
  }
}
