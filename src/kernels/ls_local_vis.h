inline void ls_local_vis(const int *p, const double *visMax,
                         const double *modal, double *viscosity) {
  ...
  double b = (*p * sum1 - sum2 * sum3) / (*p * sum4 - sum3 * sum3);
  double a = (sum2 - b * sum3) / (double)*p;
