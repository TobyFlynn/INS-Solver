inline void temperature_advec_2d_bc(const int *bedge_type, const int *edgeNum, const DG_FP *nx,
  ...
  if(*bedge_type == BC_TYPE_NO_SLIP || *bedge_type == BC_TYPE_SLIP || *bedge_type == BC_TYPE_NATURAL_OUTFLOW) {
    ...
    for(int i = 0; i < DG_NPF; i++) {
      const int fmask_ind = fmask[i];
      tP[i] = ps2d_custom_bc_get_temperature(*bedge_type, x[fmask_ind], y[fmask_ind], val[fmask_ind]);
    }
  }
  ...
  for(int i = 0; i < DG_NPF; i++) {
    ...
    DG_FP flux1 = 0.5 * (val[fmask_ind] + tP[i]);
    DG_FP flux2 = fabs(flux0);
    DG_FP flux3 = val[fmask_ind] - tP[i];
