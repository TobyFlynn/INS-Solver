...
INSData::INSData(DGMesh *m) {
  ...
  pFluxX_data    = (double *)calloc(3 * DG_NPF * mesh->numCells, sizeof(double));
  pFluxY_data    = (double *)calloc(3 * DG_NPF * mesh->numCells, sizeof(double));
  ...
  cOP_data   = (double *)calloc(DG_NP * DG_NP * mesh->numCells, sizeof(double));
  ...
  for(int i = 0; i < 3; i++) {
    ...
    gOP_data[i]  = (double *)calloc(DG_NP * DG_NP * mesh->numCells, sizeof(double));
  ...
  for(int i = 0; i < 2; i++) {
    ...
    exQ[i]      = op_decl_dat(mesh->cells, 3 * DG_NPF, "double", exQ_data[i], name.c_str());
    name        = "flux" + to_string(i);
    flux[i]     = op_decl_dat(mesh->cells, 3 * DG_NPF, "double", flux_data[i], name.c_str());
    name        = "dPdN" + to_string(i);
    dPdN[i]     = op_decl_dat(mesh->cells, 3 * DG_NPF, "double", dPdN_data[i], name.c_str());
    name        = "visRHS" + to_string(i);
    visRHS[i]   = op_decl_dat(mesh->cells, DG_NP, "double", visRHS_data[i], name.c_str());
    name        = "visBC" + to_string(i);
    visBC[i]    = op_decl_dat(mesh->cells, DG_G_NP, "double", visBC_data[i], name.c_str());
    name           = "gradCurlVel" + to_string(i);
    gradCurlVel[i] = op_decl_dat(mesh->cells, DG_NP, "double", gradCurlVel_data[i], name.c_str());
  }
  ...
  pFluxX    = op_decl_dat(mesh->cells, 3 * DG_NPF, "double", pFluxX_data, "pX");
  pFluxY    = op_decl_dat(mesh->cells, 3 * DG_NPF, "double", pFluxY_data, "pY");
  ...
  cOP   = op_decl_dat(mesh->cells, DG_NP * DG_NP, "double", cOP_data, "cub-OP");
  ...
  for(int i = 0; i < 3; i++) {
    ...
    gOP[i]      = op_decl_dat(mesh->cells, DG_NP * DG_NP, "double", gOP_data[i], name.c_str());
...
void INSData::init() {
  ...
  op2_gemm_batch(false, true, DG_NP, DG_NP, DG_CUB_NP, 1.0, Dx, DG_NP, temp, DG_NP, 0.0, cOP, DG_NP);
  op2_gemm_batch(false, true, DG_NP, DG_NP, DG_CUB_NP, 1.0, Dy, DG_NP, temp2, DG_NP, 1.0, cOP, DG_NP);
  ...
  op2_gemm(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, mDx[0], DG_GF_NP, constants->get_ptr(DGConstants::GAUSS_FINTERP0), DG_NP, 0.0, gOP[0], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, mDx[1], DG_GF_NP, mD[0], 15, 1.0, gOP[0], DG_NP);
  op2_gemm(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, mDx[2], DG_GF_NP, constants->get_ptr(DGConstants::GAUSS_FINTERP0), DG_NP, 1.0, gOP[0], DG_NP);

  op2_gemm(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, mDy[0], DG_GF_NP, constants->get_ptr(DGConstants::GAUSS_FINTERP1), DG_NP, 0.0, gOP[1], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, mDy[1], DG_GF_NP, mD[1], DG_NP, 1.0, gOP[1], DG_NP);
  op2_gemm(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, mDy[2], DG_GF_NP, constants->get_ptr(DGConstants::GAUSS_FINTERP1), DG_NP, 1.0, gOP[1], DG_NP);

  op2_gemm(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, pDx[0], DG_GF_NP, constants->get_ptr(DGConstants::GAUSS_FINTERP2), DG_NP, 0.0, gOP[2], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, pDx[1], DG_GF_NP, mD[2], DG_NP, 1.0, gOP[2], DG_NP);
  op2_gemm(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, pDx[2], DG_GF_NP, constants->get_ptr(DGConstants::GAUSS_FINTERP2), DG_NP, 1.0, gOP[2], DG_NP);
  ...
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, mDx[0], DG_GF_NP, pDy[0], DG_NP, 0.0, gOPf[0], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, mDx[1], DG_GF_NP, pD[0], DG_NP, 1.0, gOPf[0], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, mDx[2], DG_GF_NP, pDy[0], DG_NP, 1.0, gOPf[0], DG_NP);

  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, mDy[0], DG_GF_NP, pDy[1], DG_NP, 0.0, gOPf[1], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, mDy[1], DG_GF_NP, pD[1], DG_NP, 1.0, gOPf[1], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, -1.0, mDy[2], DG_GF_NP, pDy[1], DG_NP, 1.0, gOPf[1], DG_NP);

  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, pDx[0], DG_GF_NP, pDy[2], DG_NP, 0.0, gOPf[2], DG_NP);
  op2_gemm_batch(true, true, DG_NP, DG_NP, DG_GF_NP, 1.0, pDx[1], DG_GF_NP, pD[2], DG_NP, 1.0, gOPf[2], DG_NP);
