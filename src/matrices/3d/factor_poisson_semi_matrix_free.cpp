...
FactorPoissonSemiMatrixFree3D::FactorPoissonSemiMatrixFree3D(DGMesh3D *m, bool alloc_tmp_dats) : PoissonSemiMatrixFree3D(m, alloc_tmp_dats) {
...
void FactorPoissonSemiMatrixFree3D::mult(op_dat in, op_dat out) {
  ...
  #ifdef OP2_DG_CUDA
  timer->startTimer("FactorPoissonSemiMatrixFree3D - mult cells MM");
  mesh->mass(in_grad[0]);
  mesh->mass(in_grad[1]);
  mesh->mass(in_grad[2]);
  timer->endTimer("FactorPoissonSemiMatrixFree3D - mult cells MM");
  timer->startTimer("FactorPoissonSemiMatrixFree3D - mult cells Emat");
  ...
  timer->endTimer("FactorPoissonSemiMatrixFree3D - mult cells Emat");
  timer->startTimer("FactorPoissonSemiMatrixFree3D - mult cells cells");
  ...
  timer->endTimer("FactorPoissonSemiMatrixFree3D - mult cells cells");
  #else
