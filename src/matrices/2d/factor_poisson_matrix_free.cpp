#include "matrices/2d/factor_poisson_matrix_free_2d.h"
...
FactorPoissonMatrixFree2D::FactorPoissonMatrixFree2D(DGMesh2D *m) : PoissonMatrixFree2D(m) {
...
void FactorPoissonMatrixFree2D::set_factor(op_dat f) {
  factor = f;
}

void FactorPoissonMatrixFree2D::apply_bc(op_dat rhs, op_dat bc) {
  if(mesh->bface2cells) {
...
void FactorPoissonMatrixFree2D::mult(op_dat in, op_dat out) {
  timer->startTimer("FactorPoissonMatrixFree2D - mult");
  timer->startTimer("FactorPoissonMatrixFree2D - mult grad");
  mesh->grad(in, in_grad[0], in_grad[1]);
  timer->endTimer("FactorPoissonMatrixFree2D - mult grad");

  timer->startTimer("FactorPoissonMatrixFree2D - mult faces");
  ...
  timer->endTimer("FactorPoissonMatrixFree2D - mult faces");

  timer->startTimer("FactorPoissonMatrixFree2D - mult MM");
  mesh->mass(in_grad[0]);
  mesh->mass(in_grad[1]);
  timer->endTimer("FactorPoissonMatrixFree2D - mult MM");

  timer->startTimer("FactorPoissonMatrixFree2D - mult cells");
  ...
  timer->endTimer("FactorPoissonMatrixFree2D - mult cells");
  timer->endTimer("FactorFactorPoissonMatrixFree2D - mult");
}
