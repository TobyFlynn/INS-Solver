#include "matrices/2d/poisson_matrix_free_2d.h"
...
PoissonMatrixFree2D::PoissonMatrixFree2D(DGMesh2D *m) {
  mesh = m;
  _mesh = m;
  petscMatInit = false;
...
void PoissonMatrixFree2D::mult(op_dat in, op_dat out) {
  ...
  timer->startTimer("PoissonMatrixFree2D - mult grad");
  mesh->grad(in, in_grad[0], in_grad[1]);
  timer->endTimer("PoissonMatrixFree2D - mult grad");

  timer->startTimer("PoissonMatrixFree2D - mult faces");
  ...
  timer->endTimer("PoissonMatrixFree2D - mult faces");

  timer->startTimer("PoissonMatrixFree2D - mult MM");
  mesh->mass(in_grad[0]);
  mesh->mass(in_grad[1]);
  timer->endTimer("PoissonMatrixFree2D - mult MM");

  timer->startTimer("PoissonMatrixFree2D - mult cells");
...
void PoissonMatrixFree2D::apply_bc(op_dat rhs, op_dat bc) {
  ...
  timer->endTimer("PoissonMatrixFree2D - apply_bc");
}
