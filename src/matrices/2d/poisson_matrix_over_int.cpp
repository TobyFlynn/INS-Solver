...
PoissonMatrixOverInt2D::PoissonMatrixOverInt2D(DGMesh2D *m) {
  mesh = m;
  _mesh = m;
  petscMatInit = false;
...
PoissonMatrixOverInt2D::~PoissonMatrixOverInt2D() {
  if(petscMatInit)
    MatDestroy(&pMat);
}

void PoissonMatrixOverInt2D::calc_mat() {
  timer->startTimer("PoissonMatrixOverInt2D - calc_mat");
  ...
  timer->endTimer("PoissonMatrixOverInt2D - calc_mat");
}

void PoissonMatrixOverInt2D::calc_glb_ind() {
  timer->startTimer("PoissonMatrixOverInt2D - calc_glb_ind");
  set_glb_ind();
  ...
  timer->endTimer("PoissonMatrixOverInt2D - calc_glb_ind");
}

void PoissonMatrixOverInt2D::apply_bc(op_dat rhs, op_dat bc) {
  timer->startTimer("PoissonMatrixOverInt2D - apply_bc");
  if(mesh->bface2cells) {
  ...
  timer->endTimer("PoissonMatrixOverInt2D - apply_bc");
}

void PoissonMatrixOverInt2D::calc_op1() {
  timer->startTimer("PoissonMatrixOverInt2D - calc_op1");
op_par_loop(poisson_matrix_2d_op1, "poisson_matrix_2d_op1", mesh->cells,
  ...
  timer->endTimer("PoissonMatrixOverInt2D - calc_op1");
}

void PoissonMatrixOverInt2D::calc_op2() {
  timer->startTimer("PoissonMatrixOverInt2D - calc_op2");
  ...
  timer->endTimer("PoissonMatrixOverInt2D - calc_op2");
}

void PoissonMatrixOverInt2D::calc_opbc() {
  timer->startTimer("PoissonMatrixOverInt2D - calc_opbc");
  if(mesh->bface2cells) {
  ...
  timer->endTimer("PoissonMatrixOverInt2D - calc_opbc");
}
