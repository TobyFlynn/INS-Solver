...
PoissonSemiMatrixFree2D::PoissonSemiMatrixFree2D(DGMesh2D *m) {
  mesh = m;
  _mesh = m;
  petscMatInit = false;
...
void PoissonSemiMatrixFree2D::mult(op_dat in, op_dat out) {
  timer->startTimer("PoissonSemiMatrixFree2D - mult");
  timer->startTimer("PoissonSemiMatrixFree2D - mult grad");
  mesh->grad(in, in_grad[0], in_grad[1]);
  timer->endTimer("PoissonSemiMatrixFree2D - mult grad");

  timer->startTimer("PoissonSemiMatrixFree2D - mult faces");
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - mult faces");

  timer->startTimer("PoissonSemiMatrixFree2D - mult MM");
  mesh->mass(in_grad[0]);
  mesh->mass(in_grad[1]);
  timer->endTimer("PoissonSemiMatrixFree2D - mult MM");

  timer->startTimer("PoissonSemiMatrixFree2D - mult cells");
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - mult cells");
  timer->endTimer("PoissonSemiMatrixFree2D - mult");
}

void PoissonSemiMatrixFree2D::calc_mat_partial() {
  timer->startTimer("PoissonSemiMatrixFree2D - calc_mat_partial");
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - calc_mat_partial");
}

void PoissonSemiMatrixFree2D::calc_glb_ind() {
  timer->startTimer("PoissonSemiMatrixFree2D - calc_glb_ind");
  set_glb_ind();
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - calc_glb_ind");
}

void PoissonSemiMatrixFree2D::apply_bc(op_dat rhs, op_dat bc) {
  timer->startTimer("PoissonSemiMatrixFree2D - apply_bc");
  if(mesh->bface2cells) {
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - apply_bc");
}

void PoissonSemiMatrixFree2D::calc_op1() {
  timer->startTimer("PoissonSemiMatrixFree2D - calc_op1");
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - calc_op1");
}

void PoissonSemiMatrixFree2D::calc_op2() {
  timer->startTimer("PoissonSemiMatrixFree2D - calc_op2");
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - calc_op2");
}

void PoissonSemiMatrixFree2D::calc_opbc() {
  timer->startTimer("PoissonSemiMatrixFree2D - calc_opbc");
  if(mesh->bface2cells) {
  ...
  timer->endTimer("PoissonSemiMatrixFree2D - calc_opbc");
}
