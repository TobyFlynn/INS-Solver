...
FactorPoissonSemiMatrixFreeOverInt2D::FactorPoissonSemiMatrixFreeOverInt2D(DGMesh2D *m) : PoissonSemiMatrixFreeOverInt2D(m) {
...
void FactorPoissonSemiMatrixFreeOverInt2D::set_factor(op_dat f) {
  factor = f;
}
...
void FactorPoissonSemiMatrixFreeOverInt2D::mult(op_dat in, op_dat out) {
  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult");
  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult grad");
  mesh->grad(in, in_grad[0], in_grad[1]);
  timer->endTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult grad");

  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult faces");
  ...
  timer->endTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult faces");

  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult MM");
  mesh->mass(in_grad[0]);
  mesh->mass(in_grad[1]);
  timer->endTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult MM");

  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult cells");
  ...
  timer->endTimer("FactorPoissonSemiMatrixFreeOverInt2D - mult cells");
  timer->endTimer("FactorFactorPoissonSemiMatrixFreeOverInt2D - mult");
}

void FactorPoissonSemiMatrixFreeOverInt2D::calc_op1() {
  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - calc_op1");
  ...
  timer->endTimer("FactorPoissonSemiMatrixFreeOverInt2D - calc_op1");
}

void FactorPoissonSemiMatrixFreeOverInt2D::calc_op2() {
  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - calc_op2");
  op2_gemv(mesh, false, 1.0, DGConstants::GAUSS_INTERP, factor, 0.0, gFactor);
  ...
  timer->endTimer("FactorPoissonSemiMatrixFreeOverInt2D - calc_op2");
}

void FactorPoissonSemiMatrixFreeOverInt2D::calc_opbc() {
  timer->startTimer("FactorPoissonSemiMatrixFreeOverInt2D - calc_opbc");
  if(mesh->bface2cells) {
  ...
  timer->endTimer("FactorPoissonSemiMatrixFreeOverInt2D - calc_opbc");
}
