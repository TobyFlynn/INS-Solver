...
LS::LS(DGMesh *m, INSData *d) {
  ...
  q_data     = (double *)calloc((DG_ORDER_K + 1) * mesh->numCells, sizeof(double));
  ...
  q     = op_decl_dat(mesh->cells, DG_ORDER_K + 1, "double", q_data, "q");
}
...
void LS::calc_local_diff_const() {
  ...
  op2_gemv(true, DG_NP, DG_G_NP, 1.0, constants->get_ptr(DGConstants::INV_V), DG_NP, s, 0.0, modal);
  ...
  op_par_loop(ls_group_modal, "ls_group_modal", mesh->cells,
              ...
              op_arg_dat(q,     -1, OP_ID, DG_ORDER_K + 1, "double", OP_WRITE));
}
