...
//user function
__device__ void poisson_mf_cells_gpu( const double *sJ, const double *nx,
                             const double *ny, const double *qx,
                             const double *qy, double *flux) {
  for(int i = 0; i < 21; i++) {
...
__global__ void op_cuda_poisson_mf_cells(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    poisson_mf_cells_gpu(arg0+n*21,
                     arg1+n*21,
                     arg2+n*21,
                     arg3+n*21,
                     arg4+n*21,
...
//host stub function
void op_par_loop_poisson_mf_cells(char const *name, op_set set,
  op_arg arg0,
  op_arg arg1,
  op_arg arg2,
  op_arg arg3,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  poisson_mf_cells");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_poisson_mf_cells<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
