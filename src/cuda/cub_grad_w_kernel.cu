...
//user function
__device__ void cub_grad_w_gpu( double *temp0, const double *rx, const double *sx,
                     const double *ry, const double *sy, const double *J,
                     double *temp1, double *temp2, double *temp3) {
  for(int i = 0; i < 46; i++) {
    temp1[i] = cubW_g_cuda[i] * J[i] * sx[i] * temp0[i];
    temp2[i] = cubW_g_cuda[i] * J[i] * ry[i] * temp0[i];
    temp3[i] = cubW_g_cuda[i] * J[i] * sy[i] * temp0[i];
    temp0[i] = cubW_g_cuda[i] * J[i] * rx[i] * temp0[i];
  }

}

// CUDA kernel function
__global__ void op_cuda_cub_grad_w(
  double *arg0,
...
void op_par_loop_cub_grad_w(char const *name, op_set set,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  cub_grad_w");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_cub_grad_w<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
  ...
  OP_kernels[18].transfer += (float)set->size * arg7.size * 2.0f;
  OP_kernels[18].transfer += (float)set->size * arg8.size * 2.0f;
}
