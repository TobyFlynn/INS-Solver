...
//user function
__device__ void viscosity_rhs_rho_nu_gpu( const double *rho, const double *nu,
                                 double *vRHS0, double *vRHS1) {
  for(int i = 0; i < 15; i++) {
    vRHS0[i] = rho[i] * vRHS0[i] / nu[i];
    vRHS1[i] = rho[i] * vRHS1[i] / nu[i];
  }

}

// CUDA kernel function
__global__ void op_cuda_viscosity_rhs_rho_nu(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    viscosity_rhs_rho_nu_gpu(arg0+n*15,
                         arg1+n*15,
                         arg2+n*15,
                         arg3+n*15);
  }
...
//host stub function
void op_par_loop_viscosity_rhs_rho_nu(char const *name, op_set set,
  op_arg arg0,
  op_arg arg1,
  op_arg arg2,
  op_arg arg3){
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  viscosity_rhs_rho_nu");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_viscosity_rhs_rho_nu<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
