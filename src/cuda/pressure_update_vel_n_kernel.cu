...
//user function
__device__ void pressure_update_vel_n_gpu( const double *factor, const double *dpdx,
                                const double *dpdy, const double *qt0,
                                const double *qt1, double *qtt0, double *qtt1, double *nBCx, double *nBCy) {
  for(int i = 0; i < 15; i++) {
...
__global__ void op_cuda_pressure_update_vel_n(
  const double *arg0,
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    pressure_update_vel_n_gpu(arg0,
                          arg1+n*15,
                          arg2+n*15,
                          arg3+n*15,
                          arg4+n*15,
...
//host stub function
void op_par_loop_pressure_update_vel_n(char const *name, op_set set,
  op_arg arg0,
  op_arg arg1,
  op_arg arg2,
  op_arg arg3,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  pressure_update_vel_n");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_pressure_update_vel_n<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
