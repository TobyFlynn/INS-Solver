...
__global__ void op_cuda_viscosity_rhs_rho(
...
void op_par_loop_viscosity_rhs_rho(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_viscosity_rhs_rho<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
  ...
  OP_kernels[49].transfer += (float)set->size * arg1.size * 2.0f;
  OP_kernels[49].transfer += (float)set->size * arg2.size * 2.0f;
  OP_kernels[49].transfer += (float)set->size * arg3.size * 2.0f;
}
