...
//user function
__device__ void min_max_gpu( double *min, double *max, const double *arr) {
  for(int i = 0; i < 15; i++) {
    if(arr[i] > *max) {
      *max = arr[i];
    }
...
__global__ void op_cuda_min_max(
  double *arg0,
  double *arg1,
  ...
  for ( int d=0; d<1; d++ ){
    op_reduction<OP_MIN>(&arg0[d+blockIdx.x*1],arg0_l[d]);
  }
  for ( int d=0; d<1; d++ ){
    op_reduction<OP_MAX>(&arg1[d+blockIdx.x*1],arg1_l[d]);
  }
}
...
void op_par_loop_min_max(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    for ( int b=0; b<maxblocks; b++ ){
      for ( int d=0; d<1; d++ ){
        ((double *)arg0.data)[d+b*1] = arg0h[d];
      }
    }
    reduct_bytes += ROUND_UP(maxblocks*1*sizeof(double));
    ...
    op_cuda_min_max<<<nblocks,nthread,nshared>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      set->size );
    ...
    for ( int b=0; b<maxblocks; b++ ){
      for ( int d=0; d<1; d++ ){
        arg0h[d] = MIN(arg0h[d],((double *)arg0.data)[d+b*1]);
      }
    }
    ...
    for ( int b=0; b<maxblocks; b++ ){
      for ( int d=0; d<1; d++ ){
        arg1h[d] = MAX(arg1h[d],((double *)arg1.data)[d+b*1]);
      }
    }
