...
//user function
__device__ void poisson_mf_rho_nu_gpu( const double *rho, const double *nu,
                              const double *u, double *tmp) {
  for(int i = 0; i < 15; i++) {
    tmp[i] = u[i] * rho[i] / nu[i];
  }

}

// CUDA kernel function
__global__ void op_cuda_poisson_mf_rho_nu(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    poisson_mf_rho_nu_gpu(arg0+n*15,
                      arg1+n*15,
                      arg2+n*15,
                      arg3+n*15);
  }
...
void op_par_loop_poisson_mf_rho_nu(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_poisson_mf_rho_nu<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
