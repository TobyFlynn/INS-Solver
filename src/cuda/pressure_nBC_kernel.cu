...
//user function
__device__ void pressure_nBC_gpu( const double *nx, const double *ny, const double *dPdN,
...
__global__ void op_cuda_pressure_nBC(
...
void op_par_loop_pressure_nBC(char const *name, op_set set,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  pressure_nBC");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_pressure_nBC<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
  ...
  OP_kernels[10].transfer += (float)set->size * arg7.size;
  OP_kernels[10].transfer += (float)set->size * arg8.size;
}
