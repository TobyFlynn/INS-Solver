...
//user function
__device__ void poisson_mf_bc_grad_gpu( const int *bedgeType, const int *bedgeNum,
  ...
  if(*bedgeType == *d0 || *bedgeType == *d1 || *bedgeType == *d2) {
    ...
    for(int j = 0; j < 7 * 15; j++) {
      ...
      op[row * 7 + col] += -mD * gaussW_g_cuda[j % 7] * sJ[*bedgeNum * 7 + (j % 7)];
    }
...
__global__ void op_cuda_poisson_mf_bc_grad(
  ...
  if (tid + start < end) {
    ...
    poisson_mf_bc_grad_gpu(arg0+n*1,
                       arg1+n*1,
                       arg2,
                       arg3,
                       arg4,
  ...
  if (set_size > 0) {
    ...
    for ( int round=0; round<2; round++ ){
      ...
      if (end-start>0) {
        ...
        op_cuda_poisson_mf_bc_grad<<<nblocks,nthread>>>(
        (double *)arg5.data_d,
        (double *)arg6.data_d,
        (double *)arg7.data_d,
        (double *)arg8.data_d,
