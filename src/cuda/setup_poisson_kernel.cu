...
//user function
__device__ void setup_poisson_gpu( double *tau, double *bcTau, double *ex1, double *ex2) {
  for(int i = 0; i < 15; i++) {
    tau[i] = 0.0;
    bcTau[i] = 0.0;
    ex1[i] = 0.0;
...
__global__ void op_cuda_setup_poisson(
  double *arg0,
  double *arg1,
  double *arg2,
  double *arg3,
...
void op_par_loop_setup_poisson(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_setup_poisson<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
