...
//user function
__device__ void poisson_mf_nu_gpu( const double *nu, double *dudx, double *dudy) {
  for(int i = 0; i < 15; i++) {
    dudx[i] = dudx[i] * nu[i];
    dudy[i] = dudy[i] * nu[i];
  }
...
__global__ void op_cuda_poisson_mf_nu(
...
void op_par_loop_poisson_mf_nu(char const *name, op_set set,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  poisson_mf_nu");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_poisson_mf_nu<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      set->size );
