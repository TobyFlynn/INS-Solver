...
//user function
__device__ void viscosity_solve_0_gpu( const double *J, const double *Dx,
  ...
  for(int m = 0; m < 46; m++) {
    for(int n = 0; n < 15; n++) {
      int ind = m * 15 + n;
      tmpX[ind] = J[m] * cubW_g_cuda[m] * Dx[ind] * mu[m];
      tmpY[ind] = J[m] * cubW_g_cuda[m] * Dy[ind] * mu[m];
    }
  }
...
__global__ void op_cuda_viscosity_solve_0(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    viscosity_solve_0_gpu(arg0+n*46,
                      arg1+n*690,
                      arg2+n*690,
                      arg3+n*46,
                      arg4+n*15,
...
void op_par_loop_viscosity_solve_0(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_viscosity_solve_0<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
