...
//user function
__device__ void poisson_mf_rho_gpu( const double *rho, double *dudx, double *dudy) {
  for(int i = 0; i < 15; i++) {
    dudx[i] = dudx[i] / rho[i];
    dudy[i] = dudy[i] / rho[i];
  }
...
__global__ void op_cuda_poisson_mf_rho(
...
void op_par_loop_poisson_mf_rho(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_poisson_mf_rho<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      set->size );
