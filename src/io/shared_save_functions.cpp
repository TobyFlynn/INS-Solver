...
void get_cells_order_4(vector<double> &x_v, vector<double> &y_v,
                       vector<cgsize_t> &cells, double *x, double *y,
                       int numCells) {
...
void get_cells_order_3(vector<double> &x_v, vector<double> &y_v,
                       vector<cgsize_t> &cells, double *x, double *y,
                       int numCells) {
  // Maps points to sub elements that they are part of.
  // Each line is 6 long (as 6 is the max number of sub elements within an original element that a point can be part of)
...
void get_cells_order_2(vector<double> &x_v, vector<double> &y_v,
                       vector<cgsize_t> &cells, double *x, double *y,
                       int numCells) {
  // Maps points to sub elements that they are part of.
  // Each line is 6 long (as 6 is the max number of sub elements within an original element that a point can be part of)
...
void get_cells_order_1(vector<double> &x_v, vector<double> &y_v,
                       vector<cgsize_t> &cells, double *x, double *y,
                       int numCells) {
  // Maps points to sub elements that they are part of.
  // Each line is 6 long (as 6 is the max number of sub elements within an original element that a point can be part of)
...
void get_save_data_order_4(vector<double> &x_v, vector<double> &y_v,
  ...
  for(int i = 0; i < numVals; i++) {
    ...
    vals_v.push_back(empty_vec);
  }

  // 16 sub elements per original element
  map<pair<double,double>,unique_ptr<Point2>, cmpCoords> pointMap;
  for(int c = 0; c < numCells; c++) {
    int ind = c * DG_NP;
    for(int p = 0; p < DG_NP; p++) {
      pair<double,double> coords = make_pair(x[ind + p], y[ind + p]);
      unique_ptr<Point2> point = make_unique<Point2>();
      auto res = pointMap.insert(make_pair(coords, move(point)));
      if(res.second) {
        res.first->second->x = x[ind + p];
        res.first->second->y = y[ind + p];
        for(int v = 0; v < numVals; v++) {
          res.first->second->vals.push_back(vals_data[v][ind + p]);
        }
        for(int m = 0; m < 6; m++) {
          if(cellMask[p][m] >= 0) {
            res.first->second->cells.push_back(c * 16 + cellMask[p][m]);
        ...
        for(int v = 0; v < numVals; v++) {
          res.first->second->vals[v] += vals_data[v][ind + p];
        }
        for(int m = 0; m < 6; m++) {
          if(cellMask[p][m] >= 0) {
            res.first->second->cells.push_back(c * 16 + cellMask[p][m]);
  ...
  for(auto const &p : pointMap) {
    ...
    y_v.push_back(p.second->y);
    for(int v = 0; v < numVals; v++) {
      vals_v[v].push_back(p.second->vals[v] / p.second->counter);
    }
    for(int i = 0; i < p.second->cells.size(); i++) {
      cells[p.second->cells[i] * 3 + p.second->pointNum[i]] = index + 1;
    }
...
void get_save_data_order_3(vector<double> &x_v, vector<double> &y_v,
                           vector<vector<double>> &vals_v,
                           vector<cgsize_t> &cells, double *x, double *y,
                           double **vals_data, int numVals, int numCells) {
  // Maps points to sub elements that they are part of.
...
void get_save_data_order_2(vector<double> &x_v, vector<double> &y_v,
                           vector<vector<double>> &vals_v,
                           vector<cgsize_t> &cells, double *x, double *y,
                           double **vals_data, int numVals, int numCells) {
  // Maps points to sub elements that they are part of.
...
void get_save_data_order_1(vector<double> &x_v, vector<double> &y_v,
                           vector<vector<double>> &vals_v,
                           vector<cgsize_t> &cells, double *x, double *y,
                           double **vals_data, int numVals, int numCells) {
  // Maps points to sub elements that they are part of.
