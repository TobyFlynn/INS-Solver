...
Poisson_MF2::Poisson_MF2(DGMesh *m, INSData *d) : Poisson(m, d) {
...
bool Poisson_MF2::solve(op_dat b_dat, op_dat x_dat, bool addMass, double factor) {
  ...
  if(massMat) {
    ...
    op_par_loop(poisson_mf2_apply_bc_vis, "poisson_mf2_apply_bc_vis", mesh->bedges,
...
void Poisson_MF2::calc_rhs(const double *u_d, double *rhs_d) {
  ...
  if(massMat) {
    ...
    op_par_loop(poisson_mf2_vis, "poisson_mf2_vis", mesh->cells,
    ...
    op_par_loop(poisson_mf2_faces_vis, "poisson_mf2_faces_vis", mesh->edges,
                op_arg_dat(data->nu, 0, mesh->edge2cells, 15, "double", OP_READ),
                op_arg_dat(data->rho, 0, mesh->edge2cells, 15, "double", OP_READ),
                ...
                op_arg_dat(data->nu, 1, mesh->edge2cells, 15, "double", OP_READ),
                op_arg_dat(data->rho, 1, mesh->edge2cells, 15, "double", OP_READ),
