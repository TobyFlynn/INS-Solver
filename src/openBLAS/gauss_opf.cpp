...
inline void openblas_gauss_opf(const int numCells, double *op, const double *gf,
  ...
  for(int c = 0; c < numCells; c++) {
    ...
    cblas_dgemm(CblasColMajor, CblasTrans, CblasTrans, 15, 15, 7, 1.0, term0_c, 7, gf_c, 15, 0.0, op_c, 15);
    cblas_dgemm(CblasColMajor, CblasTrans, CblasTrans, 15, 15, 7, 1.0, term1_c, 7, pD_c, 15, 1.0, op_c, 15);
    cblas_dgemm(CblasColMajor, CblasTrans, CblasTrans, 15, 15, 7, -1.0, term2_c, 7, gf_c, 15, 1.0, op_c, 15);
  }
}
...
void gauss_opf_blas(INSData *nsData, GaussData *gaussData) {
  ...
  openblas_gauss_opf(nsData->numCells, (double *)gaussData->OPf[0]->data,
                   (double *)gaussData->pDy[0]->data, (double *)gaussData->pD[0]->data, (double *)gaussData->mDx[0]->data,
                   (double *)gaussData->mDx[1]->data, (double *)gaussData->mDx[2]->data);

  openblas_gauss_opf(nsData->numCells, (double *)gaussData->OPf[1]->data,
                   (double *)gaussData->pDy[1]->data, (double *)gaussData->pD[1]->data, (double *)gaussData->mDy[0]->data,
                   (double *)gaussData->mDy[1]->data, (double *)gaussData->mDy[2]->data);

  openblas_gauss_opf(nsData->numCells, (double *)gaussData->OPf[2]->data,
                   (double *)gaussData->pDy[2]->data, (double *)gaussData->pD[2]->data, (double *)gaussData->pDx[0]->data,
