...
//user function
//#pragma acc routine
inline void cub_div_openacc( double *temp0, double *temp1, const double *rx, const double *sx,
...
void op_par_loop_cub_div(char const *name, op_set set,
  ...
  OP_kernels[33].transfer += (float)set->size * arg5.size;
  OP_kernels[33].transfer += (float)set->size * arg6.size;
  OP_kernels[33].transfer += (float)set->size * arg7.size * 2.0f;
  OP_kernels[33].transfer += (float)set->size * arg8.size * 2.0f;
}
