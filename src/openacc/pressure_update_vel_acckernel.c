...
//user function
//#pragma acc routine
inline void pressure_update_vel_openacc( const double *factor, const double *dpdx,
...
void op_par_loop_pressure_update_vel(char const *name, op_set set,
  ...
  if (set_size >0) {
    ...
    #pragma acc parallel loop independent deviceptr(data1,data2,data3,data4,data5,data6,data7)
  ...
  OP_kernels[53].transfer += (float)set->size * arg5.size * 2.0f;
  OP_kernels[53].transfer += (float)set->size * arg6.size * 2.0f;
  OP_kernels[53].transfer += (float)set->size * arg7.size * 2.0f;
}
