...
inline void cublas_advection_lift(cublasHandle_t handle, const int numCells,
  ...
  double alpha = 1.0;
  double beta = 1.0;
  cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, 15, numCells, 15, &alpha, LIFT_d, 15, flux0_d, 15, &beta, N0_d, 15);
  cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, 15, numCells, 15, &alpha, LIFT_d, 15, flux1_d, 15, &beta, N1_d, 15);
  cudaFree(LIFT_d);
}
