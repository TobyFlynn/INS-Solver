...
inline void cublas_init_grid(cublasHandle_t handle, const int numCells,
  ...
  for(int c = 0; c < numCells; c++) {
    ...
    double alpha2 = 1.0;
    double beta = 0.0;
    cublasDgemv(handle, CUBLAS_OP_T, 15, 15, &alpha2, Dr_d, 15, x, 1, &beta, xr, 1);
    // xs = Ds * x
    cublasDgemv(handle, CUBLAS_OP_T, 15, 15, &alpha2, Ds_d, 15, x, 1, &beta, xs, 1);
    // yr = Dr * y
    cublasDgemv(handle, CUBLAS_OP_T, 15, 15, &alpha2, Dr_d, 15, y, 1, &beta, yr, 1);
    // ys = Ds * y
    cublasDgemv(handle, CUBLAS_OP_T, 15, 15, &alpha2, Ds_d, 15, y, 1, &beta, ys, 1);
  }
