...
inline void cublas_gauss_coords(cublasHandle_t handle, const int numCells,
  ...
  double alpha = 1.0;
  double beta = 0.0;
  cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, 21, numCells, 15, &alpha, gInterp_d, 15, x_d, 15, &beta, gx_d, 21);
  cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, 21, numCells, 15, &alpha, gInterp_d, 15, y_d, 15, &beta, gy_d, 21);
