...
inline void cublas_div(cublasHandle_t handle, const int numCells, const double *u_d,
                       ...
                       double *div2_d, double *div3_d, bool transpose) {
  double *Dr_d;
  ...
  double beta = 0.0;
  if(transpose) {
    cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, 15, numCells, 15, &alpha, Dr_d, 15, u_d, 15, &beta, div0_d, 15);
    cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, 15, numCells, 15, &alpha, Ds_d, 15, u_d, 15, &beta, div1_d, 15);
    cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, 15, numCells, 15, &alpha, Dr_d, 15, v_d, 15, &beta, div2_d, 15);
    cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, 15, numCells, 15, &alpha, Ds_d, 15, v_d, 15, &beta, div3_d, 15);
  } else {
...
void div_blas(INSData *nsData, op_dat u, op_dat v, bool transpose) {
  ...
  cublas_div(handle, nsData->numCells, (double *)u->data_d,
                   ...
                   (double *)nsData->div[3]->data_d, transpose);
