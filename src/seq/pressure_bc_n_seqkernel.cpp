...
void op_par_loop_pressure_bc_n(char const *name, op_set set,
  ...
  OP_kernels[8].transfer += (float)set->size * arg11.size * 2.0f;
  OP_kernels[8].transfer += (float)set->size * arg12.size * 2.0f;
  ...
  OP_kernels[8].transfer += (float)set->size * arg3.map->dim * 4.0f;
}
