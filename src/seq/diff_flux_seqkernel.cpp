...
void op_par_loop_diff_flux(char const *name, op_set set,
  ...
  OP_kernels[81].name      = name;
  OP_kernels[81].count    += 1;
  OP_kernels[81].time     += wall_t2 - wall_t1;
  OP_kernels[81].transfer += (float)set->size * arg2.size;
  OP_kernels[81].transfer += (float)set->size * arg4.size;
  OP_kernels[81].transfer += (float)set->size * arg6.size;
  OP_kernels[81].transfer += (float)set->size * arg8.size;
  OP_kernels[81].transfer += (float)set->size * arg10.size;
  OP_kernels[81].transfer += (float)set->size * arg12.size * 2.0f;
  OP_kernels[81].transfer += (float)set->size * arg0.size;
  OP_kernels[81].transfer += (float)set->size * arg1.size;
  OP_kernels[81].transfer += (float)set->size * arg2.map->dim * 4.0f;
}
