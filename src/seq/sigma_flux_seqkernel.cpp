...
void op_par_loop_sigma_flux(char const *name, op_set set,
  ...
  OP_kernels[66].transfer += (float)set->size * arg13.size * 2.0f;
  OP_kernels[66].transfer += (float)set->size * arg15.size * 2.0f;
  OP_kernels[66].transfer += (float)set->size * arg0.size;
  OP_kernels[66].transfer += (float)set->size * arg1.map->dim * 4.0f;
}
