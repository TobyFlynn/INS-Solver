...
void op_par_loop_sigma_flux(char const *name, op_set set,
  ...
  OP_kernels[65].transfer += (float)set->size * arg6.size;
  OP_kernels[65].transfer += (float)set->size * arg8.size;
  OP_kernels[65].transfer += (float)set->size * arg10.size * 2.0f;
  OP_kernels[65].transfer += (float)set->size * arg12.size * 2.0f;
  OP_kernels[65].transfer += (float)set->size * arg0.size;
  OP_kernels[65].transfer += (float)set->size * arg1.size;
  OP_kernels[65].transfer += (float)set->size * arg2.map->dim * 4.0f;
}
