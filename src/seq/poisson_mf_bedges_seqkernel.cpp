...
void op_par_loop_poisson_mf_bedges(char const *name, op_set set,
  ...
  OP_kernels[31].transfer += (float)set->size * arg12.size * 2.0f;
  OP_kernels[31].transfer += (float)set->size * arg13.size * 2.0f;
  ...
  OP_kernels[31].transfer += (float)set->size * arg5.map->dim * 4.0f;
}
