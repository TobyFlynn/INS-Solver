...
//user function
#include "../kernels/pressure_solve_apply_bc.h"
...
void op_par_loop_pressure_solve_apply_bc(char const *name, op_set set,
  ...
  OP_kernels[26].transfer += (float)set->size * arg8.size;
  ...
  OP_kernels[26].transfer += (float)set->size * arg12.size;
  OP_kernels[26].transfer += (float)set->size * arg13.size;
  ...
  OP_kernels[26].transfer += (float)set->size * arg5.map->dim * 4.0f;
}
