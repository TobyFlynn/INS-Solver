...
void op_par_loop_ls_flux(char const *name, op_set set,
  ...
  OP_kernels[57].transfer += (float)set->size * arg5.size;
  OP_kernels[57].transfer += (float)set->size * arg7.size;
  OP_kernels[57].transfer += (float)set->size * arg9.size;
  OP_kernels[57].transfer += (float)set->size * arg11.size;
  OP_kernels[57].transfer += (float)set->size * arg13.size * 2.0f;
  OP_kernels[57].transfer += (float)set->size * arg15.size * 2.0f;
  OP_kernels[57].transfer += (float)set->size * arg17.size * 2.0f;
  OP_kernels[57].transfer += (float)set->size * arg19.size * 2.0f;
  OP_kernels[57].transfer += (float)set->size * arg0.size;
  OP_kernels[57].transfer += (float)set->size * arg1.map->dim * 4.0f;
}
