...
void op_par_loop_ls_flux(char const *name, op_set set,
  ...
  OP_kernels[71].transfer += (float)set->size * arg12.size * 2.0f;
  OP_kernels[71].transfer += (float)set->size * arg14.size * 2.0f;
  ...
  OP_kernels[71].transfer += (float)set->size * arg2.map->dim * 4.0f;
}
