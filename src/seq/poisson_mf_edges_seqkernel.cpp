...
void op_par_loop_poisson_mf_edges(char const *name, op_set set,
  ...
  OP_kernels[31].transfer += (float)set->size * arg16.size * 2.0f;
  OP_kernels[31].transfer += (float)set->size * arg18.size * 2.0f;
  OP_kernels[31].transfer += (float)set->size * arg20.size * 2.0f;
  OP_kernels[31].transfer += (float)set->size * arg0.size;
  OP_kernels[31].transfer += (float)set->size * arg1.size;
  OP_kernels[31].transfer += (float)set->size * arg2.map->dim * 4.0f;
}
