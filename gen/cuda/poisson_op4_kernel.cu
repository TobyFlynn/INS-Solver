...
//user function
__device__ void poisson_op4_gpu( const double *cJ, const double *factor, const double *fact, double *op, double *tmp) {
  double cTmp[36 * 10];
  double mm[10 * 10];
  for(int m = 0; m < 36; m++) {
    for(int n = 0; n < 10; n++) {
  ...
  for(int i = 0; i < 10; i++) {
    for(int j = 0; j < 10; j++) {
      ...
      for(int k = 0; k < 36; k++) {
        ...
        mm[c_ind] += cubV_g_cuda[a_ind] * cTmp[b_ind];
      }
    }
  }
...
void op_par_loop_poisson_op4(char const *name, op_set set,
  ...
  OP_kernels[52].transfer += (float)set->size * arg3.size * 2.0f;
  OP_kernels[52].transfer += (float)set->size * arg4.size * 2.0f;
}
