...
__device__ void init_gauss_grad_neighbour_gpu( const int *reverse, double *rx,
  ...
  if(reverse[1]) {
    for(int m = 0; m < 3; m++) {
      for(int n = 0; n < 3; n++) {
        int ind = m * 3 + n;
        Dx1[ind] = rx[m + 3] * gF1DrR_g_cuda[ind] + sx[m + 3] * gF1DsR_g_cuda[ind];
        Dy1[ind] = ry[m + 3] * gF1DrR_g_cuda[ind] + sy[m + 3] * gF1DsR_g_cuda[ind];
      }
    }
  } else {
    for(int m = 0; m < 3; m++) {
      for(int n = 0; n < 3; n++) {
        int ind = m * 3 + n;
        Dx1[ind] = rx[m + 3] * gF1Dr_g_cuda[ind] + sx[m + 3] * gF1Ds_g_cuda[ind];
        Dy1[ind] = ry[m + 3] * gF1Dr_g_cuda[ind] + sy[m + 3] * gF1Ds_g_cuda[ind];
      }
    }
  }
  ...
  if(reverse[2]) {
    for(int m = 0; m < 3; m++) {
      for(int n = 0; n < 3; n++) {
        int ind = m * 3 + n;
        Dx2[ind] = rx[m + 2 * 3] * gF2DrR_g_cuda[ind] + sx[m + 2 * 3] * gF2DsR_g_cuda[ind];
        Dy2[ind] = ry[m + 2 * 3] * gF2DrR_g_cuda[ind] + sy[m + 2 * 3] * gF2DsR_g_cuda[ind];
      }
    }
  } else {
    for(int m = 0; m < 3; m++) {
      for(int n = 0; n < 3; n++) {
        int ind = m * 3 + n;
        Dx2[ind] = rx[m + 2 * 3] * gF2Dr_g_cuda[ind] + sx[m + 2 * 3] * gF2Ds_g_cuda[ind];
        Dy2[ind] = ry[m + 2 * 3] * gF2Dr_g_cuda[ind] + sy[m + 2 * 3] * gF2Ds_g_cuda[ind];
      }
    }
  }
