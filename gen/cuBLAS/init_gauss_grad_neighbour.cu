...
inline void cublas_init_gauss_grad_neighbour(cublasHandle_t handle, const int numCells, const int *reverse,
  ...
  for(int c = 0; c < numCells; c++) {
    ...
    // Face 2
    if(reverse[3 * c + 2]) {
      cublasDgemv(handle, CUBLAS_OP_T, 15, 7, &alpha, constants->gF2DrR_d, 15, x, 1, &beta, gxr + 2 * 7, 1);
      cublasDgemv(handle, CUBLAS_OP_T, 15, 7, &alpha, constants->gF2DsR_d, 15, x, 1, &beta, gxs + 2 * 7, 1);
      cublasDgemv(handle, CUBLAS_OP_T, 15, 7, &alpha, constants->gF2DrR_d, 15, y, 1, &beta, gyr + 2 * 7, 1);
      cublasDgemv(handle, CUBLAS_OP_T, 15, 7, &alpha, constants->gF2DsR_d, 15, y, 1, &beta, gys + 2 * 7, 1);
    } else {
