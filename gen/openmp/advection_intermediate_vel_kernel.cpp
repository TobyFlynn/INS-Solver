...
void op_par_loop_advection_intermediate_vel(char const *name, op_set set,
  ...
  OP_kernels[31].transfer += (float)set->size * arg14.size;
  OP_kernels[31].transfer += (float)set->size * arg15.size;
  OP_kernels[31].transfer += (float)set->size * arg16.size;
  OP_kernels[31].transfer += (float)set->size * arg17.size;
  OP_kernels[31].transfer += (float)set->size * arg18.size * 2.0f;
  OP_kernels[31].transfer += (float)set->size * arg19.size * 2.0f;
}
