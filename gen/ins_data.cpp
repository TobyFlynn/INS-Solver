...
INSData::INSData(DGMesh *m) {
  ...
  for(int i = 0; i < 2; i++) {
    ...
    exQ_data[i]    = (double *)calloc(3 * 5 * mesh->numCells, sizeof(double));
    flux_data[i]   = (double *)calloc(3 * 5 * mesh->numCells, sizeof(double));
    dPdN_data[i]   = (double *)calloc(3 * 5 * mesh->numCells, sizeof(double));
  ...
  pFluxX_data    = (double *)calloc(3 * 5 * mesh->numCells, sizeof(double));
  pFluxY_data    = (double *)calloc(3 * 5 * mesh->numCells, sizeof(double));
  ...
  for(int i = 0; i < 2; i++) {
    ...
    exQ[i]      = op_decl_dat(mesh->cells, 3 * 5, "double", exQ_data[i], name.c_str());
    name        = "flux" + to_string(i);
    flux[i]     = op_decl_dat(mesh->cells, 3 * 5, "double", flux_data[i], name.c_str());
    name        = "dPdN" + to_string(i);
    dPdN[i]     = op_decl_dat(mesh->cells, 3 * 5, "double", dPdN_data[i], name.c_str());
  ...
  pFluxX    = op_decl_dat(mesh->cells, 3 * 5, "double", pFluxX_data, "pX");
  pFluxY    = op_decl_dat(mesh->cells, 3 * 5, "double", pFluxY_data, "pY");
...
void INSData::init() {
  ...
  op_par_loop(gauss_tau_bc, "gauss_tau_bc", mesh->bedges,
              ...
              op_arg_dat(mesh->fscale,    0, mesh->bedge2cells, 3 * 5, "double", OP_READ),
