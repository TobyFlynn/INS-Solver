...
//user function
#include "../kernels/init_gauss_grad4_2.h"
...
void op_par_loop_init_gauss_grad4_2(char const *name, op_set set,
  ...
  if (OP_diags>2) {
    printf(" kernel routine with indirection: init_gauss_grad4_2\n");
  }
  ...
  OP_kernels[47].transfer += (float)set->size * arg5.size;
  OP_kernels[47].transfer += (float)set->size * arg6.size;
  OP_kernels[47].transfer += (float)set->size * arg7.size;
  OP_kernels[47].transfer += (float)set->size * arg8.size;
  OP_kernels[47].transfer += (float)set->size * arg9.size;
  OP_kernels[47].transfer += (float)set->size * arg0.size;
  OP_kernels[47].transfer += (float)set->size * arg10.size;
  OP_kernels[47].transfer += (float)set->size * arg1.map->dim * 4.0f;
}
