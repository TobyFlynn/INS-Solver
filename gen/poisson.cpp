...
PoissonSolve::PoissonSolve(DGMesh *m, INSData *d, bool p) {
  ...
  cmmFactor_data = (double *)calloc(36 * mesh->numCells, sizeof(double));
  ...
  cmmFactor = op_decl_dat(mesh->cells, 36, "double", cmmFactor_data, "poisson_cmmFactor");
...
void PoissonSolve::set_op() {
  ...
  op_par_loop(poisson_op2, "poisson_op2", mesh->edges,
              op_arg_dat(mesh->edgeNum,  -1, OP_ID, 2, "int", OP_READ),
              op_arg_dat(mesh->reverse,  -1, OP_ID, 1, "bool", OP_READ),
              op_arg_dat(data->mDL,      -1, OP_ID, 6 * 10, "double", OP_READ),
              op_arg_dat(data->mDR,      -1, OP_ID, 6 * 10, "double", OP_READ),
              op_arg_dat(data->pDL,      -1, OP_ID, 6 * 10, "double", OP_READ),
              op_arg_dat(data->pDR,      -1, OP_ID, 6 * 10, "double", OP_READ),
              op_arg_dat(data->gVPL,     -1, OP_ID, 6 * 10, "double", OP_READ),
              op_arg_dat(data->gVPR,     -1, OP_ID, 6 * 10, "double", OP_READ),
  ...
  op_par_loop(poisson_op3, "poisson_op3", mesh->bedges,
              ...
              op_arg_dat(data->mDBC,     -1, OP_ID, 6 * 10, "double", OP_READ),
  ...
  if(massMat) {
    op_par_loop(poisson_op4, "poisson_op4", mesh->cells,
                op_arg_dat(mesh->cubature->J, -1, OP_ID, 36, "double", OP_READ),
                op_arg_dat(cmmFactor, -1, OP_ID, 36, "double", OP_READ),
                op_arg_dat(factor,    -1, OP_ID, 10, "double", OP_READ),
...
void PressureSolve::setup() {
  ...
  // for(int i = 0; i < 10 * mesh->numCells; i++) {
  //   for(int j = i + 1; j < 10 * mesh->numCells; j++) {
  //     double l[1];
  //     double r[1];
...
void ViscositySolve::setup(double mmConst) {
  ...
  op2_gemv(true, 36, 10, 1.0, constants->get_ptr(DGConstants::CUB_V), 10, factor, 0.0, cFactor);
  op2_gemv(true, 18, 10, 1.0, constants->get_ptr(DGConstants::GAUSS_INTERP), 10, factor, 0.0, gFactor);
  op2_gemv(true, 36, 10, 1.0, constants->get_ptr(DGConstants::CUB_V), 10, mmFactor, 0.0, cmmFactor);
  set_op();
