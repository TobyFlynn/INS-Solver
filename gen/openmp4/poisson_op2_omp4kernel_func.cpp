    ...
    for(int i = 0; i < 10; i++) {
      for(int j = 0; j < 10; j++) {
        ...
        for(int k = 0; k < 6; k++) {
          ...
          op1L[c_ind] += -0.5 * mDL[a_ind] * gaussW_g_ompkernel[k]
                         * sJL[factors_indL] * gVML[b_ind];
          op1R[c_ind] += -0.5 * mDR[a_ind] * gaussW_g_ompkernel[k]
                         * sJR[factors_indR] * gVMR[b_ind];

          op2L[c_ind] += 0.5 * mDL[a_ind] * gaussW_g_ompkernel[k]
                         * sJL[factors_indL] * gVPL[b_ind];
          op2R[c_ind] += 0.5 * mDR[a_ind] * gaussW_g_ompkernel[k]
