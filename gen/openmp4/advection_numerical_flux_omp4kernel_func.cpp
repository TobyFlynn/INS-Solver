...
void advection_numerical_flux_omp4_kernel(
  ...
  for ( int n_op=0; n_op<count; n_op++ ){
    ...
    double *flux0 = &data5[12*n_op];
    double *flux1 = &data6[12*n_op];
    ...
    for(int i = 0; i < 3 * 4; i++) {
      flux0[i] = 0.5 * fscale[i] * (-nx[i] * (fM[0][i] - fP[0][i]) - ny[i] * (fM[1][i] - fP[1][i]) - maxVel[i] * (flux0[i] - q0[FMASK_ompkernel[i]]));
      flux1[i] = 0.5 * fscale[i] * (-nx[i] * (fM[2][i] - fP[2][i]) - ny[i] * (fM[3][i] - fP[3][i]) - maxVel[i] * (flux1[i] - q1[FMASK_ompkernel[i]]));
    }
    //end inline func
  }
